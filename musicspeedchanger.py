...
@loader.tds
class MusicSpeedChangerMod(loader.Module):
    ...
    async def ratecmd(self, message):
        ...
        if reply and reply.file and reply.file.mime_type.split("/")[0] == "audio":
            ...
            await message.edit('[Music Speed Changer] Ответь на аудио!')
        ...
        if reply and reply.file and reply.file.mime_type.split("/")[0] == "audio":
            if args:
                ...
                if args[0] == 'nc' or 'nightcore':
                    ...
                    await message.edit('[Music Speed Changer] Работаю...')
                    if performer or title:
                        outputfile = f"{performer or title}_(nightcore)_{random_letters}.mp3"
                    else:
                        outputfile = f"audio_(nightcore)_{random_letters}.mp3"
                    if performer or title:
                        os.system(f'ffmpeg -i "{file_name}" -filter:a "aresample=44100,asetrate=44100*1.48" -b:a 320k -metadata artist="{performer}" -metadata title="{title} (nightcore)" "{outputfile}"')
                    else:
                        os.system(f'ffmpeg -i "{file_name}" -filter:a "aresample=44100,asetrate=44100*1.48" -b:a 320k "{outputfile}"')
                    fileinfo = f'{outputfile}'
                    audiofile = eyed3.load(fileinfo)
                    song_length_seconds = audiofile.info.time_secs
                    duration = int(song_length_seconds)
                    await message.edit('[Music Speed Changer] Отправляю...')
                    await message.client.send_file(message.to_id, f'{outputfile}', voice_note=True, attributes=[types.DocumentAttributeAudio(duration=duration, performer=f'{performer}', title=f'{title} (nightcore)')])
                    ...
                    await message.edit('[Music Speed Changer] Такого эффекта не существует...')
            else:
                await message.edit('[Music Speed Changer] Доступные эффекты\n\n⏩nc или nightcore - NightCore')
        else:
            await message.edit('[Music Speed Changer] Доступные эффекты\n\n⏩nc или nightcore - NightCore')
